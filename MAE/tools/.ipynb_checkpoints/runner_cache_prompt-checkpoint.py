...
def run_net(args, config, config_cp, train_writer=None, val_writer=None):
    ...
    for name, param in base_model.named_parameters():
        if 'cache_gate' in name or 'cls_pos' in name or 'cls_token' in name or 'proj.bias' in name or 'fc2.bias' in name or 'fc1.bias' in name or 'norm2.bias' in name or 'norm1.bias' in name or 'cls_head_' in name or "norm." in name or ".adapter" in name or "adapter1" in name or "out_transform" in name or "norm3" in name or "attn1." in name or "prompt_embeddings" in name or "cp" in name or ".gate" in name or "ad_gate" in name: #or "norm3" in name: #or  "deep_prompt_embeddings_1" in name  or "deep_prompt_embeddings_2" in name: #or "ln1" in name: #or "ln2" in name: #or "norm3" in name or "attn1." in name: # or "norm_linear" in name:'proj.bias' in name or 'fc2.bias' in name or 'fc1.bias' in name or 'norm2.bias' in name or 'norm1.bias' in name or 
            #'cls_pos' in name or 'cls_token' in name or
    ...
    for name,param in base_model.named_parameters():
        ...
        if param.requires_grad:
            ...
            if 'cls_pos' in name:
                ...
                cp_nums += param.numel()
            elif 'cache_gate' in name:
                cache_nums += param.numel()
            elif ".gate" in name:
                ...
                encoder_nums += param.numel()
            elif 'pos_embed' in name:
                pos_embed_nums += param.numel()
            # elif 'ln1' in name:
            #     ln1_nums += param.numel()
            # elif 'ln2' in name:
            #     ln2_nums += param.numel()
    ...
    print('parameters:',total_para_nums,'adapter',adapter_para_nums,'norm',norm_para_nums,'cls_head',head_para_nums, 'out_transform',out_transform_nums,'norm_linear',norm_linear_nums, "norm3", norm3_nums,"attn1_nums", attn1_nums,
          "ln1_nums", ln1_nums,"ln2_nums", ln2_nums, "prompt", prompt_nums, "cp", cp_nums, "gate", gate_nums, "ad_gate", ad_gate_nums, 'cls_token', cls_token_nums, 'bias', bias_num, 'encoder', encoder_nums, 'pos_embed', pos_embed_nums, 'cache_gate', cache_nums)
