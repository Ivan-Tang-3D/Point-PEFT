...
class ConvBNReLURes2D(nn.Module):
    def __init__(self, channel, kernel_size=1, groups=1, res_expansion=1.0, bias=True, activation='relu', last=False):
        ...
        if not last:
            ...
            self.net1 = nn.Sequential(
                nn.Conv2d(in_channels=channel*2, out_channels=int(channel * res_expansion),
                        kernel_size=kernel_size, groups=groups, bias=bias),
                nn.BatchNorm2d(int(channel * res_expansion)),
                self.act
            )
...
class PosExtraction(nn.Module):
    def __init__(self, channels, blocks=1, groups=1, res_expansion=1, bias=True, activation='relu'):
        ...
        for _ in range(blocks):
            operation.append(
                ConvBNReLURes2D(channels, groups=groups, res_expansion=res_expansion, bias=bias, activation=activation)
                # ConvBNReLU2D(channels, channels, bias=False, activation=activation)
            )
...
class PosPool_Block(nn.Module):
    ...
    def forward(self, xyz, re_xyz, x):
        ...
        #     channel = x.shape[1]
        #     x = x[:,:int(channel/2),:,:]
        # x = self.transfer(x).reshape(B, -1, npoint, nsample)
...
class PosPool_Layer(nn.Module):
    ...
    def forward(self, re_xyz, x):
        ...
        # aggregation_features *= position_embedding  # (B, C, npoint, nsample)
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.points = points
        # self.embedding = InitPosPool(3, embed_dim)

        self.embedding = ConvBNReLU1D(3, embed_dim, bias=False, activation=activation)
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            # print("stage", i, ":")
            # torch.save(xyz, "/root/gzy/visual/featuremap_PN/xyz_stage" + str(i) + ".pt")
            # torch.save(x, "/root/gzy/visual/featuremap_PN/x_stage" + str(i) + ".pt")
        ...
        # global count
        # torch.save(x, "/root/PointMAE/PointMLP/pn_scan/feat_train_" + str(count) + ".pt")
        

        # global count
        # torch.save(x, "/root/PointMAE/PointMLP/mlp_scan/feat_" + str(count) + ".pt")
        x = self.classifier(x)
        # torch.save(x, "/root/PointMAE/PointMLP/pn_scan/feat_label_" + str(count) + ".pt")
        # count += 1
        return x
...
def pointPOS_st(num_classes=40, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=36, groups=1, res_expansion=0.5,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 1], pos_blocks=[2, 1, 1, 1],
                   k_neighbors=[40] * 4, reducers=[2, 2, 2, 2], **kwargs)

# def pointPOS_st(num_classes=40, **kwargs) -> Model:
#     return Model(points=1024, class_num=num_classes, embed_dim=84, groups=1, res_expansion=0.75,
#                    activation="relu", bias=False, use_xyz=False, normalize="anchor",
#                    dim_expansion=[2, 2, 2, 1], pos_blocks=[2, 2, 2, 2],
#                    k_neighbors=[24] * 4, reducers=[2, 2, 2, 2], **kwargs)


def pointPOS_st_scan(num_classes=15, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=84, groups=1, res_expansion=0.75,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 1], pos_blocks=[2, 2, 2, 2],
                   k_neighbors=[24] * 4, reducers=[2, 2, 2, 2], **kwargs)
...
def pointPOS_st_scan_e(num_classes=15, **kwargs) -> Model:
    return Model(points=1024, class_num=num_classes, embed_dim=36, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
